...
def train(args: ExperimentArgs, device='cuda'):
    ...
    for metric in GoalPredictorMetrics.names():
        ...
        logger.define_metric('test_unseen_' + metric, 'epoch')
    ...
    test_unseen_episodes = generate_episodes_from_mturk_trajectories(objects,
                                                                    load_mturk_trajectories('test_unseen', 'all',
                                                                                            args.altitude))
    ...
    for epoch in trange(start_epoch, args.epochs, desc='epochs', unit='epoch', colour='#448844'):
        ...
        if (epoch + 1) % args.eval_every == 0:
            _eval_predictor_and_log_metrics(goal_predictor, val_seen_episodes, val_unseen_episodes, test_unseen_episodes, args, device)
...
def _eval_predictor_and_log_metrics(
    ...
    test_unseen_episodes: list[Episode],
    args: ExperimentArgs,
    device: str,
):
    ...
    test_unseen_metrics = eval_goal_predictor(args, test_unseen_episodes,
                                             *run_episodes_batch(args, goal_predictor, test_unseen_episodes, device))
    ...
    logger.log({'test_unseen_' + k: v for k, v in test_unseen_metrics.to_dict().items()})
