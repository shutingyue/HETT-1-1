...
class ActionPredictionHead(nn.Module):
    ...
    def __init__(self, n_map_features: int, n_rgb_features: int, n_depth_featuers):
        ...
        self.prediction_head = nn.Sequential(
            nn.Linear(n_map_features + n_rgb_features + n_depth_featuers + 512, 512),
            nn.ReLU(),
            nn.Linear(512, 256),
            nn.ReLU(),
            nn.Linear(256, 4),
    ...
    def forward(self, map_features, rgb_features, depth_features, direction_features, position_features, target_features):
        return self.prediction_head(torch.cat((map_features, rgb_features, depth_features, direction_features, position_features, target_features), dim=1))
...
class GoalPredictor(nn.Module):
    ...
    def __init__(self, map_size: int):
        ...
        self.action_prediction_head = ActionPredictionHead(
            self.map_encoder.out_features, self.rgb_encoder.out_features, self.depth_encoder.out_features
        )
        self.direction_embedding = nn.Linear(2, 256)
        self.position_embedding = nn.Linear(2, 128)
        self.target_embedding = nn.Linear(2, 128)
    
    def forward(self, maps: Tensor, rgbs: Tensor, depths: Tensor, directions: Tensor, positions: Tensor, flip_depth=True):
        """rgb & depth (B, C, H, W)"""
        ...
        direction_features = self.direction_embedding(directions)
        position_features = self.position_embedding(positions)
        target_features = self.target_embedding(pred_normalized_goal_xys)
        pred_action = self.action_prediction_head(map_features, rgb_features, depth_features, direction_features, position_features, target_features)

        return pred_normalized_goal_xys, pred_action
    ...
    def predict(
        ...
        pred_normalized_goal_coords, pred_action = self(maps, rgb, depth, flip_depth)
        pred_xy = to_world_xy(pred_normalized_goal_coords)
        return pred_xy, pred_action