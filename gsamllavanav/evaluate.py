...
def eval_goal_predictor(
    ...
    for eps in episodes:
        trajectory_lengths = np.sum([a.xy.dist_to(b.xy) for a, b in zip(trajectory_logs[eps.id][:-1], trajectory_logs[eps.id][1:])])
        gt_net_lengths = trajectory_logs[eps.id][0].xy.dist_to(eps.target_position.xy)
        success = float(trajectory_logs[eps.id][-1].xy.dist_to(eps.target_position.xy) <= args.success_dist)
        spl = success * gt_net_lengths / max(trajectory_lengths, gt_net_lengths, 0.01)
        spls.append(spl)
    ...
    for episodes_batch in tqdm(dataloader, desc='eval episodes', unit='batch', colour='#88dd88', position=1):
        ...
        nav_maps = [
            LandmarkNavMap(
                ...
                eps.description_landmarks, eps.description_target, eps.description_surroundings
        ...
        for t in trange(args.eval_max_timestep, desc='eval timestep', unit='step', colour='#66aa66', position=2, leave=False):
            ...
            dones = dones | (pred_progresses.cpu().numpy().flatten() >= 1)
