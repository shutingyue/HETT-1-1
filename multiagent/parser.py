...
def parse_args():
    ...
    add_bool_flag('debug_anomaly', default=False)
    ...
    add_bool_flag('debug_elam', default=False)
    parser.add_argument('--num_elam_roles', type=int, default=4)
    parser.add_argument('--elam_num_heads', type=int, default=8)
    parser.add_argument('--elam_dropout', type=float, default=0.1)
    parser.add_argument('--elam_fusion_mode', type=str, choices=['none', 'add', 'gate'], default='none')
    parser.add_argument('--elam_aux_weight', type=float, default=1.0)
    parser.add_argument('--metric_cell_loss_weight', type=float, default=1.0)
    parser.add_argument('--soft_spatial_loss_weight', type=float, default=0.5)
    parser.add_argument('--query_div_loss_weight', type=float, default=0.1)
    parser.add_argument('--soft_spatial_sigma', type=float, default=0.15)
    add_bool_flag('use_uasc', default=False)
    parser.add_argument('--uasc_hidden_size', type=int, default=None)
    parser.add_argument('--uasc_dropout', type=float, default=0.1)
    parser.add_argument('--uasc_aux_dim', type=int, default=9)
    parser.add_argument('--uasc_lambda_conf', type=float, default=0.2)
    parser.add_argument('--uasc_lambda_stage', type=float, default=0.2)
    parser.add_argument('--uasc_lambda_stop', type=float, default=0.5)
    parser.add_argument('--uasc_lambda_calib', type=float, default=0.1)
    add_bool_flag('use_uasc_calib', default=False)
    add_bool_flag('uasc_debug', default=False)
    parser.add_argument(
        '--uasc_control_mode',
        type=str,
        choices=['aux_only', 'stop_only', 'stage_stop'],
        default='aux_only',
    )
    parser.add_argument('--uasc_tau_stop', type=float, default=0.7)
    parser.add_argument('--uasc_tau_conf', type=float, default=0.5)
    parser.add_argument('--uasc_tau_stage', type=float, default=0.5)
    parser.add_argument('--uasc_stage_radius', type=float, default=30.0)
...
def postprocess_args(args):
    ...
    args.use_elam = bool(args.use_elam)
    if args.use_elam and not args.use_umti:
        raise ValueError("ELAM requires use_umti=True because it aligns language queries to UMTI memory tokens.")
    args.num_elam_roles = max(int(args.num_elam_roles), 1)
    args.elam_num_heads = max(int(args.elam_num_heads), 1)
    args.elam_dropout = float(args.elam_dropout)
    args.elam_aux_weight = float(args.elam_aux_weight)
    args.metric_cell_loss_weight = float(args.metric_cell_loss_weight)
    args.soft_spatial_loss_weight = float(args.soft_spatial_loss_weight)
    args.query_div_loss_weight = float(args.query_div_loss_weight)
    args.soft_spatial_sigma = float(args.soft_spatial_sigma)
    args.use_uasc = bool(args.use_uasc)
    args.uasc_hidden_size = int(args.uasc_hidden_size if args.uasc_hidden_size is not None else args.demb)
    args.uasc_aux_dim = max(int(args.uasc_aux_dim), 1)
    args.uasc_dropout = float(args.uasc_dropout)
    args.uasc_lambda_conf = float(args.uasc_lambda_conf)
    args.uasc_lambda_stage = float(args.uasc_lambda_stage)
    args.uasc_lambda_stop = float(args.uasc_lambda_stop)
    args.uasc_lambda_calib = float(args.uasc_lambda_calib)
    args.use_uasc_calib = bool(args.use_uasc_calib)
    args.uasc_debug = bool(args.uasc_debug)
    args.uasc_stage_radius = float(args.uasc_stage_radius)
