...
flag="--world_size ${ngpus}
      ...
      --max_action_len 20
      --grid_size 5
      --checkpoint /cver/xcding/code/citynav/checkpoints/multi/best_val_unseen
      "
...
CUDA_VISIBLE_DEVICES='4' python -m torch.distributed.run --nnodes=$NNODES --node_rank=$NODE_RANK --master_addr=$MASTER_ADDR --nproc_per_node=$ngpus --master_port=$PORT \
  main.py $flag
