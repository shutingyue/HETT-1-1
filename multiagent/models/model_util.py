...
def generate_attention_mask(len_lang, len_frames, len_positions, device):
    """
    ...
    lang_to_rest = torch.ones((len_lang, len_frames + 2 + len_positions), device=device).float() * float("-inf")
    lang_to_all = torch.cat((lang_to_lang, lang_to_rest), dim=1)

    frames_to_all = torch.zeros((len_frames, len_lang + len_frames + 2 + len_positions), device=device).float()
    # frames_to_rest = torch.ones((1, 2 + len_positions), device=device).float()
    # frames_to_all = torch.cat((frames_to_lang, frames_to_rest), dim=1)
    ...
    # frames_to_objects = torch.zeros((len_frames), device=device).float()
    ...
    # actions_to_lang = torch.zeros((1, len_lang + 2), device=device).float()
    # actions_to_rest = torch.ones((1, 1 + len_positions), device=device).float()
    # actions_to_all = torch.cat((actions_to_lang, actions_to_rest), dim=1)
    actions_to_all = torch.zeros((1, len_lang + len_frames + 2 + len_positions), device=device).float()

    # maps_to_lang = torch.ones((1, len_lang + 2), device=device).float()
    # maps_to_rest = torch.zeros((1, 1 + len_positions), device=device).float()
    # maps_to_all = torch.cat((maps_to_lang, maps_to_rest), dim=1)
    maps_to_all = actions_to_all.clone()
    # centroids_to_all = actions_to_all.clone()

    positions_to_all = torch.zeros((len_positions, len_lang + len_frames + 2 + len_positions), device=device).float()
    # centroids_to_all = torch.zeros((len_centroids, len_lang + 3 + len_positions + len_centroids), device=device).float()

    # 4. concatenate all the masks
    all_to_all = torch.cat((lang_to_all, frames_to_all, actions_to_all, maps_to_all, positions_to_all), dim=0)
    return all_to_all


# def generate_attention_mask(len_lang, len_positions, device):
#     """
#     generate mask for attention (a timestep at t does not attend to timesteps after t)"""
#     # 1. language should attend only to language
#     lang_to_lang = torch.zeros((len_lang, len_lang), device=device).float()
#     lang_to_rest = torch.ones((len_lang, 3 + len_positions), device=device).float() * float("-inf")
#     lang_to_all = torch.cat((lang_to_lang, lang_to_rest), dim=1)
#
#     frames_to_all = torch.zeros((1, len_lang + 3 + len_positions), device=device).float()
#     # 2.1 frames should attend to all language tokens
#     # frames_to_lang = torch.zeros((len_frames, len_lang), device=device).float()
#     # # 2.2 frames should attend to frames with timestep <= t
#     # frames_to_frames = triangular_mask(len_frames, device)
...
#     maps_to_all = frames_to_all.clone()
#
#     positions_to_all = torch.zeros((len_positions, len_lang + 3 + len_positions), device=device).float()
#
#     # objects_to_lang = torch.zeros((len_objects, len_lang), device=device).float()
#     # objects_to_frames1 = torch.ones((len_objects, len_frames - 1), device=device).float() * float("-inf")
#     # objects_to_frames2 = torch.zeros((len_objects, 1), device=device).float()
#     # objects_to_frames = torch.cat((objects_to_frames1, objects_to_frames2), dim=1)
#     # objects_to_directions = objects_to_frames.clone()
#     # objects_to_objects = torch.zeros((len_objects, len_objects), device=device).float()
#     # objects_to_all = torch.cat((objects_to_lang, objects_to_frames, objects_to_directions, objects_to_objects), dim=1)
#
#     # 4. concatenate all the masks
#     all_to_all = torch.cat((lang_to_all, frames_to_all, actions_to_all, maps_to_all, positions_to_all), dim=0)
#     return all_to_all
