...
from direction.defaultpaths import GOAL_PREDICTOR_CHECKPOINT_DIR
...
def train(args, train_env, train_full_traj_env, val_envs, val_full_traj_envs, aug_env=None, rank=-1):
    ...
    if args.checkpoint is not None:
        start_iter = agent.load(os.path.join(args.checkpoint))
        if default_gpu:
            write_to_record_file(
                "\nLOAD the model from {}, iteration {}".format(args.checkpoint, start_iter),
                record_file
            )
    ...
    for idx in range(start_iter, start_iter + args.iters, interval):
        ...
        agent.save(iter, os.path.join(GOAL_PREDICTOR_CHECKPOINT_DIR, "latest"))
